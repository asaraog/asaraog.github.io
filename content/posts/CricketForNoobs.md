...
- **Neural narration.** Read-aloud streams a neural voice from an LLM-based text-to-speech model, with the browser's built-in voice as an automatic mid-sentence fallback. LLM-based TTS will improvise words when fed text truncated mid-thought, so speech is chunked into whole sentences only.
