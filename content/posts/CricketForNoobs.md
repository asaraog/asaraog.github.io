...
- **Two model providers, one voice.** Gemini's free tier writes the narrative and serves about 98% of LLM turns in production. The server fails over to Groq's Llama automatically when Gemini errors or opens with a refusal, because neither Groq's acceptable-use policy nor Meta's Llama policy restricts explaining betting markets. Groq also hosts the neural voice behind read-aloud.
