...
The same model and archive are published separately as [mcp-cricket](https://github.com/asaraog/mcp-cricket), an open-source [Model Context Protocol](https://modelcontextprotocol.io) server, so any AI assistant can query them directly. It ships eleven read-only tools as a single static binary, with no API key.
