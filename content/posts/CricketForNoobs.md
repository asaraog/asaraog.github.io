...
A first-innings score means nothing except relative to what the ground usually yields, and two of the seven first-innings features are measured against par. Par was a single global constant for every T20 ever played. It is now a table of 371 grounds and seven leagues, built from the same ball-by-ball archive, falling back ground to league to global so an unrecognised spelling degrades softly rather than failing: live feeds and the archive disagree on plenty of ground names, and a hard miss there would be worse than the constant it replaces. Real pars run from 153.7 to 172.5 by league alone, and further by ground. The same 80 for 2 at ten overs is 47% at Chinnaswamy and 60% at Newlands. Held-out log loss on T20 first innings moved from 0.603 to 0.597.

The multi-day model needed the same treatment for a different reason. It priced a weaker side leading by 153 in the second innings of a Test at 15% while the market had 54%. Conditioning the archive properly found the fault: first innings was calibrated at 16% against an actual 16%, third innings said 18% where the true rate was 37%, and the second innings had 28 supporting states in the entire archive, so that case was pure extrapolation. One flat team-rating term meant first innings, which holds the most states and the least reason to discount a lead, was setting the penalty for all three. Per-innings interactions took held-out multinomial loss from 0.862 to 0.803 across 132,368 states, and that position now prices at 52%.

## The same model as an MCP server

The analytics are also published as an open-source [Model Context Protocol](https://modelcontextprotocol.io) server, [mcp-cricket](https://github.com/asaraog/mcp-cricket), so any AI assistant can call them: Claude Desktop, Claude Code, Cursor, or anything else that speaks MCP. It ships eleven read-only tools over the same fitted win model and the same ball-by-ball archive, plus career and batter-vs-bowler records, phase and venue splits, live scores, and live market prices. One static binary, JSON-RPC over stdio, no API key and no account.

The division of labour is the reason it exists. No language model can compute a win probability. Asked who is favoured at 149 for 7 chasing 178, it will produce a confident number derived from nothing. The assistant chooses which tool to call and with what arguments; the arithmetic stays in Go, where it is tested and versioned. The tool descriptions are the interface to the model rather than developer documentation, so they carry the traps: the balls-per-over argument notes that The Hundred bowls five-ball sets, because a client that silently passes six reports every required rate a fifth too low.
